...
const translations = {
  fr: {
    ...
    mobile_cart_selected: 'photos sélectionnées',
  ...
  en: {
    ...
    mobile_cart_selected: 'photos selected',
  ...
  es: {
    ...
    mobile_cart_selected: 'fotos seleccionadas',
...
function updateInterfaceLanguage() {
  ...
  // Mettre à jour le select mobile si présent
...
// VERSION SIMPLIFIÉE - Repartir de zéro
function setupMobileHeaderScroll() {
  ...
  if (window.innerWidth > 768) {
    ...
    if (header) {
      ...
      // Nettoyer tous les styles inline sur desktop
  ...
  // DEBUG: Vérifier que la fonction s'exécute
  console.log('[Header Scroll] Initialisation mobile header scroll, width:', window.innerWidth);
  
  // Ajouter un indicateur de debug visuel dans le header
  if (!document.getElementById('header-debug-indicator')) {
    const debugDiv = document.createElement('div');
    debugDiv.id = 'header-debug-indicator';
    debugDiv.style.cssText = 'position: absolute; top: 0; right: 0; background: red; color: white; padding: 2px 5px; font-size: 10px; z-index: 10000;';
    debugDiv.textContent = 'Header Debug';
  ...
  while (parent && parent !== document.body) {
    ...
    if (computed.transform !== 'none' || computed.perspective !== 'none' || computed.filter !== 'none') {
      console.warn('[Header Scroll] Parent avec transform/perspective/filter détecté:', parent.className, 'transform:', computed.transform);
      hasTransformParent = true;
    }
    parent = parent.parentElement;
  }
  
  // Si un parent a un transform, sortir le header du container (comme pour les modals)
  if (hasTransformParent || header.parentElement.classList.contains('container')) {
    console.log('[Header Scroll] Sortir le header du container pour éviter les problèmes de position: fixed');
    const container = header.parentElement;
    // Sauvegarder la position actuelle
    const nextSibling = header.nextSibling;
    // Déplacer le header directement dans body
    document.body.insertBefore(header, document.body.firstChild);
    console.log('[Header Scroll] Header déplacé dans body');
  }
  
  // S'assurer que le header est visible au chargement
  header.classList.remove('header-hidden');
  
  // Ajouter un attribut data pour augmenter la spécificité CSS
  header.setAttribute('data-mobile-header', 'true');
  
  // Utiliser setProperty avec 'important' pour forcer TOUS les styles (priorité absolue)
  const headerHeight = header.offsetHeight || 70;
  
  // Fonction pour forcer position: fixed (à appeler plusieurs fois si nécessaire)
  const forceFixedPosition = () => {
    // Forcer position: fixed IMMÉDIATEMENT avec setProperty et 'important'
    ...
    // Vérifier IMMÉDIATEMENT que position: fixed est bien appliqué
    const computedPosition = window.getComputedStyle(header).position;
    console.log('[Header Scroll] Position après forceFixedPosition:', computedPosition);
    return computedPosition === 'fixed';
  };
  ...
  if (!isFixed) {
    setTimeout(() => {
      isFixed = forceFixedPosition();
      if (!isFixed) {
        console.error('[Header Scroll] ERREUR: Impossible de forcer position: fixed');
        // Fallback: utiliser position normale sans important
  ...
  // Fonction pour changer la position du header
  const setHeaderPosition = (translateY) => {
    // FORCER avec setProperty et important
    header.style.setProperty('transform', `translateY(${translateY})`, 'important');
    console.log('[Header Scroll] ▶ Transform appliqué:', `translateY(${translateY})`);
    
    // Ajouter un indicateur visuel temporaire pour debug
    const indicator = document.getElementById('header-debug-indicator');
    if (indicator) {
      indicator.textContent = `Transform: ${translateY}`;
    }
  };
  
  // Initialiser avec translateY(0) = header visible
  setHeaderPosition('0');
  
  const handleScroll = (event) => {
    // Vérifier à nouveau qu'on est toujours sur mobile
    if (window.innerWidth > 768) {
      header.classList.remove('header-hidden');
      header.style.cssText = '';
      return;
    ...
    // Utiliser plusieurs méthodes pour obtenir le scrollTop (compatibilité)
    let scrollTop = 0;
    const container = document.querySelector('.container');
    
    // Priorité : event.target > container > window
    if (event && event.target && event.target.scrollTop !== undefined && event.target.scrollTop > 0) {
      scrollTop = event.target.scrollTop;
      console.log('[Header Scroll] scrollTop depuis event.target:', scrollTop);
    } else if (container && container.scrollTop > 0) {
      scrollTop = container.scrollTop;
      console.log('[Header Scroll] scrollTop depuis container:', scrollTop);
    } else {
      scrollTop = window.pageYOffset 
        ...
        || 0;
      if (scrollTop > 0) {
        console.log('[Header Scroll] scrollTop depuis window:', scrollTop);
      }
    }
    ...
    if (Math.abs(scrollDelta) > 0.1) {
      console.log('[Header Scroll] 📊 scrollTop:', scrollTop.toFixed(1), 'Δ:', scrollDelta.toFixed(1), 'last:', lastScrollTop.toFixed(1), 'hidden?', header.classList.contains('header-hidden'));
    }
    
    // Éviter de cacher le header tout en haut de la page
    if (scrollTop < 50) {
      // En haut : toujours visible
      if (header.classList.contains('header-hidden')) {
        console.log('[Header Scroll] ⬆️ MONTRER (en haut, scrollTop < 50)');
        header.classList.remove('header-hidden');
        setHeaderPosition('0');
      }
    } else if (scrollDelta > 5) {
      // Scroll vers le bas : cacher (seuil augmenté à 5 pour éviter faux positifs)
      if (!header.classList.contains('header-hidden')) {
        console.log('[Header Scroll] ⬇️ CACHER (scroll bas, Δ:', scrollDelta.toFixed(1), ')');
        header.classList.add('header-hidden');
        setHeaderPosition(`-${headerHeight}px`);
      }
    } else if (scrollDelta < -5) {
      // Scroll vers le haut : montrer (seuil augmenté à -5 pour être plus fiable)
      if (header.classList.contains('header-hidden')) {
        console.log('[Header Scroll] ⬆️ MONTRER (scroll haut, Δ:', scrollDelta.toFixed(1), ')');
        header.classList.remove('header-hidden');
        setHeaderPosition('0');
      }
    }
    
    // TOUJOURS mettre à jour lastScrollTop pour détecter le prochain changement de direction
    lastScrollTop = scrollTop <= 0 ? 0 : scrollTop;
  };
  
  // Utiliser requestAnimationFrame pour optimiser les performances
  // ticking est déjà déclaré plus haut, on le réutilise
  ...
  // Détecter l'élément qui scroll réellement
  // Ça peut être window, document, body, container, etc.
  const scrollContainer = document.querySelector('.container') || document.body;
  
  console.log('[Header Scroll] Élément de scroll détecté:', scrollContainer.className || scrollContainer.tagName);
  
  // Attacher le listener sur window ET sur le container (au cas où)
  ...
  // Tester immédiatement pour vérifier que ça fonctionne
  handleScroll();
  ...
  setTimeout(() => {
    ...
    const containerHeight = document.querySelector('.container')?.scrollHeight || 0;
    
    console.log('[Header Scroll] Debug hauteurs:');
    console.log('  - document.documentElement.scrollHeight:', docHeight);
    console.log('  - document.body.scrollHeight:', bodyHeight);
    console.log('  - container.scrollHeight:', containerHeight);
    console.log('  - window.innerHeight:', viewportHeight);
    console.log('  - Scroll possible?', docHeight > viewportHeight);
    
    // Si le scroll n'est pas possible, logger un avertissement
    if (docHeight <= viewportHeight) {
      console.warn('[Header Scroll] ATTENTION: Le scroll n\'est pas possible car la page n\'est pas assez longue!');
      console.warn('[Header Scroll] Ajoutez du contenu pour tester le scroll du header.');
    }
  ...
  window.addEventListener('resize', () => {
    ...
    resizeTimeout = setTimeout(() => {
      if (window.innerWidth > 768) {
        ...
        // Re-initialiser sur mobile
        header.classList.remove('header-hidden');
        setHeaderPosition('0');
        handleScroll(); // Re-tester le scroll
      }
    }, 100);
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  resetActivity(); // Initialiser le timeout d'activité
  ...
  document.querySelector('.right-column').addEventListener('click', (e) => {
    const col = e.currentTarget;
    if (col.classList.contains('collapsed')) {
      toggleColumn('right');
    }
...
// Ouvrir le modal des promotions
...
// Fonction utilitaire pour obtenir le champ de recherche actif et sa boîte de suggestions
function getActiveSearchElements() {
  const isMobile = window.innerWidth <= 768;
  const headerSearch = document.querySelector('.header-search #photo-search');
  const contentSearch = document.querySelector('.search-section #photo-search');
  const headerSuggestions = document.querySelector('.header-search #suggestions-box');
  const contentSuggestions = document.querySelector('.search-section #suggestions-box');
  
  // Sur desktop : priorité au header, sur mobile : priorité au contenu
  if (isMobile) {
    return {
      searchInput: contentSearch || headerSearch,
      suggestionsBox: contentSuggestions || headerSuggestions
    };
  } else {
    return {
      searchInput: headerSearch || contentSearch,
      suggestionsBox: headerSuggestions || contentSuggestions
    };
  }
}

// Fonction pour attacher les listeners à un champ de recherche
function attachSearchListeners(searchInput, suggestionsBox) {
  if (!searchInput || !suggestionsBox) return;
  
  let timeout;
  
  // Listener pour les suggestions - passer le champ source pour trouver la bonne boîte
  searchInput.addEventListener('input', (e) => {
    ...
    timeout = setTimeout(() => {
      fetchSuggestions(query, e.target); // Passer le champ source
    }, 300);
  });
  ...
  // Fermer les suggestions quand on clique en dehors (sur mobile surtout)
  // Utiliser une fonction nommée pour pouvoir la retirer si nécessaire
  ...
  // Attacher le listener une seule fois au document
  if (!window.suggestionsClickOutsideHandler) {
    window.suggestionsClickOutsideHandler = true;
    document.addEventListener('click', closeSuggestionsOnClickOutside, true);
  }
}

// Recherche
function setupEventListeners() {
  // Attacher les listeners aux deux champs (header et contenu) pour être sûr que ça fonctionne
  const headerSearch = document.querySelector('.header-search #photo-search');
  const contentSearch = document.querySelector('.search-section #photo-search');
  const headerSuggestions = document.querySelector('.header-search #suggestions-box');
  const contentSuggestions = document.querySelector('.search-section #suggestions-box');
  
  // Attacher aux deux champs pour que ça fonctionne sur desktop (header) et mobile (contenu)
  if (headerSearch && headerSuggestions) {
    attachSearchListeners(headerSearch, headerSuggestions);
  }
  
  if (contentSearch && contentSuggestions) {
    attachSearchListeners(contentSearch, contentSuggestions);
  }
  
  if (!headerSearch && !contentSearch) {
    console.error('Aucun champ de recherche trouvé');
  }

  document.getElementById('pack-btn').addEventListener('click', handleBuyPack);
  ...
  document.getElementById('lightbox-add-btn').addEventListener('click', toggleCurrentLightboxPhoto);
  ...
  // Cart events
  document.getElementById('validate-cart-btn').addEventListener('click', showPersonalInfoModal);
  document.getElementById('save-cart-btn').addEventListener('click', saveCartForLater);
  
  // Event listener pour le champ de recherche de code panier (Entrée) - géré inline dans le HTML
  
  // Gestion checkbox professionnel
  document.getElementById('is-professional').addEventListener('change', function() {
...
async function fetchSuggestions(query, targetInput = null) {
  // Trouver la boîte de suggestions associée au champ qui a déclenché l'événement
  let suggestionsBox = null;
  let searchInput = null;
  
  if (targetInput) {
    // Si on a le champ source, trouver sa boîte de suggestions
    const parent = targetInput.closest('.search-box, .header-search-box');
    if (parent) {
      suggestionsBox = parent.querySelector('#suggestions-box');
      searchInput = targetInput;
    }
  }
  
  // Fallback : utiliser getActiveSearchElements
  if (!suggestionsBox) {
    const elements = getActiveSearchElements();
    suggestionsBox = elements.suggestionsBox;
    searchInput = elements.searchInput;
  }
  
  if (!suggestionsBox) {
    console.error('Boîte de suggestions non trouvée');
    return;
  }
  ...
  try {
    ...
    // Capturer les références pour la closure
    const boxRef = suggestionsBox;
    const inputRef = searchInput;
    
    // Fonction pour gérer la sélection d'une suggestion
    const handleSuggestionClick = (e, index) => {
      e.preventDefault();
      e.stopPropagation();
      
      const selectedValue = currentSuggestions[index] || '';
      
      // Fermer immédiatement les suggestions AVANT toute autre action
      boxRef.style.display = 'none';
      boxRef.innerHTML = '';
      currentSuggestions = [];
      
      // Appliquer la suggestion au bon champ
      if (inputRef) {
        inputRef.value = selectedValue;
        // Petit délai pour s'assurer que le DOM est mis à jour
        setTimeout(() => {
          searchPhotos(selectedValue);
        }, 10);
      } else {
    ...
    suggestionsBox.querySelectorAll('.suggestion-item').forEach((item, idx) => {
      ...
      // Sur mobile, utiliser touchstart ET click pour être sûr
...
function applySuggestion(value) {
  // Trouver le champ de recherche actif selon le contexte
  const { searchInput, suggestionsBox } = getActiveSearchElements();
  ...
  // Mettre à jour le champ et lancer la recherche
...
function renderPhotos(photos) {
  ...
  sortedPhotos.forEach(photo => {
    ...
    // Click sur la carte = ouvrir lightbox
...
function toggleCartItem(filename, btn) {
  ...
  const card = btn ? btn.closest('.photo-card') : document.querySelector(`.photo-card[data-filename="${filename}"]`);
  const badge = card ? card.querySelector('.photo-in-cart-badge') : null;
  const addBtn = card ? card.querySelector('.photo-add-btn') : null;
  ...
  // Animation sur la bottom bar mobile ou le bouton header
...
function removePhotoFromCart(filename) {
  ...
  // Mettre à jour l'UI de la grille photos
    ...
    if (item && item.type === 'photo') {
      ...
      if (formatsToRemove.length > 0 && item.formats) {
        ...
        if (Object.keys(item.formats).length === 0) {
          ...
          // Mettre à jour l'UI de la grille photos
          document.querySelectorAll('.photo-card').forEach(card => {
            if (card.dataset.filename === item.filename) {
              card.classList.remove('in-cart');
              const addBtn = card.querySelector('.photo-add-btn');
              const badge = card.querySelector('.photo-in-cart-badge');
              if (addBtn) addBtn.style.display = 'flex';
...
function updateCartUI() {
  ...
  // Mettre à jour la bottom bar mobile
  updateMobileCartBar();
...
// Portail : s'assurer que les modals sont enfants directs de <body>
// Fix pour problème n°1 : parent avec transform
...
function toggleCart() {
  ...
  if (modal.classList.contains('active')) {
    ...
    // Débloquer le scroll du body quand le panier se ferme
    document.documentElement.classList.remove('modal-open');
    document.body.classList.remove('modal-open');
  }
}
...
function renderCartItems() {
  ...
  cart.forEach((item, index) => {
    ...
    if (item.type === 'photo') {
      ...
      if (!riderName) {
      const photoData = currentSearchResults.find(p => p.filename === item.filename);
      if (photoData) {
        riderName = photoData.rider_name || photoData.cavalier || '';
        horseName = photoData.horse_name || photoData.cheval || '';
        ...
        if (product.pricing_rules && typeof product.pricing_rules === 'object' && !useReducedPrice) {
          ...
          if (hasNumericKeys || hasDefault) {
            ...
            getPriceForPosition = (position) => {
              ...
              // Ex: "2=1" → toutes les 2 photos, 1 est offerte (positions 2, 4, 6, 8...)
              if (promoGroupSize && promoFreeCount) {
                const groupe = Math.floor((position - 1) / promoGroupSize);
                const posInGroup = ((position - 1) % promoGroupSize) + 1;
                // Les Y dernières photos du groupe sont gratuites
                if (posInGroup > (promoGroupSize - promoFreeCount)) {
                  return 0; // Gratuit
                }
          ...
          // Pas de pricing_rules ou prix réduit avec impression : utiliser basePrice ou promo gratuite
          if (useReducedPrice) {
            // Prix réduit avec impression prend le dessus
            currentPrice = product.reduced_price_with_print;
            nextPrice = product.reduced_price_with_print;
          } else if (promoGroupSize && promoFreeCount) {
            // Pas de pricing_rules, mais appliquer quand même la promo "X=Y" (logique en boucle)
            // Vérifier si la prochaine photo est gratuite
            const groupeNext = Math.floor((nextPhotoPosition - 1) / promoGroupSize);
            const posInGroupNext = ((nextPhotoPosition - 1) % promoGroupSize) + 1;
            if (posInGroupNext > (promoGroupSize - promoFreeCount)) {
              currentPrice = 0; // Gratuit
            } else {
              // Si ce n'est pas gratuit, utiliser le prix de base (pas de prix moyen)
              currentPrice = basePrice;
            }
            // Vérifier si la photo suivante est gratuite
            const groupeFollowing = Math.floor((followingPhotoPosition - 1) / promoGroupSize);
            const posInGroupFollowing = ((followingPhotoPosition - 1) % promoGroupSize) + 1;
            if (posInGroupFollowing > (promoGroupSize - promoFreeCount)) {
              nextPrice = 0; // Gratuit
            } else {
              // Si ce n'est pas gratuit, utiliser le prix de base (pas de prix moyen)
              nextPrice = basePrice;
        ...
        if (qty > 0) {
          ...
          if (!isBlockedByPack) {
            ...
            if (useReducedPrice) {
              ...
              // Utiliser getPriceForPosition si disponible, sinon utiliser basePrice
              if (getPriceForPosition) {
                actualPrice = getPriceForPosition(thisPhotoPosition);
              } else {
                // Pas de pricing_rules : vérifier si cette position est gratuite
                if (promoGroupSize && promoFreeCount) {
                  const groupe = Math.floor((thisPhotoPosition - 1) / promoGroupSize);
                  const posInGroup = ((thisPhotoPosition - 1) % promoGroupSize) + 1;
                  if (posInGroup > (promoGroupSize - promoFreeCount)) {
                    actualPrice = 0; // Gratuit
                  } else {
      ...
      const imageUrl = getPhotoUrlFromFilename(item.filename);
      
      row.innerHTML = `
        <div class="cart-photo-container">
            <img src="${imageUrl}" class="cart-photo-large" onclick="openLightbox('${item.filename}', null, true)" onload="detectCartPhotoOrientation(this)" onerror="this.style.display='none'; console.error('Image load error:', '${imageUrl}')">
...
function updateCartTotal() {
  ...
  if (product.pricing_rules && typeof product.pricing_rules === 'object' && !useReducedPrice) {
      ...
      if (hasNumericKeys || hasDefault) {
        ...
        // Calculer les photos gratuites une seule fois pour le totalQty (logique en boucle)
        let photosGratuites = [];
        if (promoGroupSize && promoFreeCount) {
          for (let pos = 1; pos <= totalQty; pos++) {
            const groupe = Math.floor((pos - 1) / promoGroupSize);
            const posInGroup = ((pos - 1) % promoGroupSize) + 1;
            // Les Y dernières photos du groupe sont gratuites
            if (posInGroup > (promoGroupSize - promoFreeCount)) {
              photosGratuites.push(pos);
            }
          }
        }
        ...
        const getPriceForPosition = (position) => {
          // Vérifier si cette position est dans la liste des photos gratuites
          if (photosGratuites.includes(position)) {
            return 0; // Gratuit
          }
      ...
      // Pas de règles définies : prix standard, mais appliquer quand même la promo "X=Y"
      ...
      for (let i = 1; i <= totalQty; i++) {
        // Si cette position est dans la liste des photos gratuites
        if (photosGratuites.includes(i)) {
          lineTotal += 0; // Gratuit
        } else {
...
function openLightbox(startFilename, photosList = null, fromCart = false) {
  ...
  if (fromCart) {
      ...
      const cartPhotos = cart
        ...
        .map(i => {
          const photoData = currentSearchResults.find(p => p.filename === i.filename);
          return {
            filename: i.filename,
            imageUrl: getPhotoUrlFromFilename(i.filename),
            displayName: i.filename.split('/').pop(),
            rider_name: photoData ? (photoData.rider_name || photoData.cavalier || '') : '',
            horse_name: photoData ? (photoData.horse_name || photoData.cheval || '') : ''
          };
        });
      ...
      lightboxPhotos = (photosList || []).map(photo => ({
  ...
  currentLightboxIndex = lightboxPhotos.findIndex(p => p.filename === startFilename);
...
function getPriceForPosition(product, position, hasPrintForSamePhoto = false) {
  ...
  // Si dans la promo gratuite (logique en boucle)
  // Ex: "2=1" → toutes les 2 photos, 1 est offerte (positions 2, 4, 6, 8...)
  if (promoGroupSize && promoFreeCount) {
    const groupe = Math.floor((position - 1) / promoGroupSize);
    const posInGroup = ((position - 1) % promoGroupSize) + 1;
  ...
  // Si on a un prix réduit avec impression, l'utiliser directement (prend le dessus sur les tarifs dégressifs)
  if (useReducedPrice) {
    return product.reduced_price_with_print;
  }
...
function resetInterface() {
    ...
    // Réinitialiser la recherche (utiliser le champ actif)
    const { searchInput, suggestionsBox } = getActiveSearchElements();
    if (searchInput) {
        searchInput.value = '';
    }
...
async function saveCartForLater() {
    ...
    try {
        const response = await fetch('/api/client/carts/save', {
            method: 'POST',
            headers: {
...
async function loadSavedCart() {
    ...
    try {
        const response = await fetch(`/api/client/carts/${code}`);
